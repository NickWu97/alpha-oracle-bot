...
【v2 新增技術分析 — 提升勝率至 70-80%】
  ...
  ⑦ Confluence Score           — 8 項評分，≥ 5 才進場（過濾低品質訊號）
...
def calculate_confluence_score(
    ...
    ③ M15 流動性掃蕩      → 1分
    ④ M5 MSS + FVG        → 1分
    ⑤ M15 Order Block 匯流 → 1分（新增）
    ⑥ OTE 最佳進場區間    → 1分（新增）
    ⑦ Silver Bullet 時段  → 1分（Silver Bullet 比其他時段多1分）
    ⑧ 資費率方向一致      → 1分（輔助確認）
    ...
    try:
        ...
        for instId in ALL_COINS:
            ...
            if instId not in active_ids:
                ...
                msg += f"🎯 方向：{side_zh}\n"
                msg += f"🌟 匯流評分：{s['score']}/8 {score_stars}\n"
                msg += f"\n"
                msg += f"📐 *多時框分析*\n"
