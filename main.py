...
MAX_DAILY_SL    = 2      # 每日止損上限
MAX_CONCURRENT  = 3      # 最大同時持倉（WAITING+ACTIVE）
SL_MIN_PCT      = 0.007  # 0.7% 最小止損距離
COOLDOWN_BARS_SL = 8     # 止損後冷卻 8 棒 = 2 小時
COOLDOWN_BARS_TP = 4     # 止盈後冷卻 4 棒 = 1 小時
...
def calculate_structural_sl(df, side, entry, atr):
    """
    止損計算。
    優先使用結構 SL（OB/FVG 低點 + buffer），
    若距離 < SL_MIN_PCT，改用 ATR*2.0。
    """
    buffer  = atr * 0.25
    min_atr = atr * 2.0
...
def get_fixed_r_tps(entry, sl, side):
    """
    TP 結構：
    TP1 = 0.7R  → 容易觸及，觸及後立即移 SL 到成本（保本）
    TP2 = 1.5R  → 中間目標，觸及後 SL 移到 TP1（鎖利）
    TP3 = 3.0R  → 最大目標

    為什麼 TP1 = 0.7R 而非 1R？
    0.7R 大約有 65-70% 機率可以達到，而 1R 只有約 50%。
    TP1 觸及後立即保本，讓「最差結果＝保本」，理論勝率大幅提升。
    """
    risk = abs(entry - sl) + 1e-10
    if side == "LONG":
        return entry + risk*0.7, entry + risk*1.5, entry + risk*3.0
    else:
        return entry - risk*0.7, entry - risk*1.5, entry - risk*3.0
...
def detect_liquidity_sweep(df, side, lookback=40) -> bool:
    """
    流動性掃蕩偵測（最高質量 SMC 訊號）
    ────────────────────────────────────────
    機構操作模式：
    1. 讓散戶止損單堆積在近期高/低點
    2. 突然把價格推到那個位置，觸發大量止損（掃蕩流動性）
    3. 立即反轉，開始真正的行情

    做多掃蕩特徵：
    - 近期 30-40 棒內有明顯低點
    - 最近 3-5 根 K 棒的低點「刺穿」了那個低點（掃蕩）
    - 但最新收盤價回到了低點上方（反轉確認）
    ...
    reference = df.iloc[:-5]   # 排除最近 5 根作為參考區
    recent    = df.iloc[-5:]   # 最近 5 根看掃蕩行為

    if side == "LONG":
        # 近期參考低點（散戶 SL 堆積區）
        ref_low = reference['l'].tail(lookback).min()
        # 最近 5 根有沒有刺破這個低點
        swept   = recent['l'].min() < ref_low * 1.002   # 允許 0.2% 誤差
        # 最新收盤必須回到低點上方（反轉）
        recovered = df['c'].iloc[-1] > ref_low
        # 掃蕩幅度不能太大（太大說明是真跌破，不是掃蕩）
        sweep_depth = (ref_low - recent['l'].min()) / (ref_low + 1e-10)
        not_too_deep = sweep_depth < 0.02   # 不超過 2%
        return swept and recovered and not_too_deep

    else:  # SHORT
        ref_high  = reference['h'].tail(lookback).max()
        swept     = recent['h'].max() > ref_high * 0.998
        recovered = df['c'].iloc[-1] < ref_high
        sweep_depth = (recent['h'].max() - ref_high) / (ref_high + 1e-10)
        not_too_deep = sweep_depth < 0.02
        return swept and recovered and not_too_deep
...
def check_rsi_filter(df, side):
    """
    RSI 過濾（稍微放寬避免過濾掉強趨勢）
    強趨勢中 RSI 可以維持在 70 以上很長時間，
    太嚴格的 RSI 限制會錯過最好的趨勢初段。
    """
    if len(df)<20: return True
    rsi = calculate_rsi(df)
    if side=="LONG"  and not (30 <= rsi <= 75): return False
    if side=="SHORT" and not (25 <= rsi <= 70): return False
    return True
...
def check_1h_trend(instId, side):
    ...
    label = "📈 1H多頭" if st==1 else ("📉 1H空頭" if st==-1 else "⚪ 1H中性")
...
def check_4h_trend(instId, side):
    ...
    label = f"{st_label} EMA({ema_gap:+.1f}%)"
...
def check_btc_4h_trend(btc_df_4h, side):
    ...
    label = f"BTC4H {'📈' if st==1 else ('📉' if st==-1 else '⚪')}"
    if side == "LONG"  and st == -1: return False, label
    if side == "SHORT" and st ==  1: return False, label
    return True, label
...
def is_trading_session(now_tw: datetime) -> tuple[bool, str]:
    """
    交易時段過濾
    ✅ 歐洲盤 15:00~00:00 + 美國盤 21:00~06:00
    ❌ 亞洲盤 06:00~15:00（低流動性，假突破多）
    """
    h = now_tw.hour
    if 6 <= h < 15:
...
def find_smc_setup(df: pd.DataFrame) -> dict | None:
    ...
    模式 A：流動性掃蕩反轉（最高優先）
    ─────────────────────────────────
    機構掃 SL 後立即反轉，是 SMC 中最可靠的形態。
    特徵：近期低點被刺穿後立即反彈收回（做多），或高點被掃後反轉（做空）。

    模式 B：BOS 結構突破 + 假突破過濾
    ─────────────────────────────────
    傳統 BOS 訊號，加入防假突破確認。
    要求突破棒強勢（實體>50%，量>1.2倍均量），
    且下一根 K 棒不跌回突破點。
    """
    ...
    for side in ["LONG", "SHORT"]:
        if detect_liquidity_sweep(df, side):
            if side == "LONG":
                # 進場在流動性低點之上（確認已反轉）
                sweep_low = df['l'].iloc[-5:].min()
                entry = df['c'].iloc[-1]   # 當前價進場（動作要快）
            else:
                sweep_high = df['h'].iloc[-5:].max()
                entry = df['c'].iloc[-1]
            best = {"side": side, "entry": entry, "setup_type": "🎯 流動性掃蕩"}
            break

    # ── 模式 B：BOS 突破（流動性掃蕩找不到時）─────────────────
    if best is None:
        for i in range(len(df)-3, len(df)-25, -1):
            ...
            if k2['c'] > k2['o'] and k2['c'] > df['h'].iloc[i-15:i].max():
                # 防假突破：下一根 K 棒收盤不跌回突破點
                breakthrough_level = df['h'].iloc[i-15:i].max()
                if i + 2 < len(df):
                    k3 = df.iloc[i + 2]
                    if k3['c'] < breakthrough_level:
                        continue   # 假突破
                entry = k2['l'] if k2['l'] > k0['h'] else k1['c']
                best  = {"side": "LONG", "entry": entry, "setup_type": "📈 BOS 突破"}
                ...
                best  = {"side": "SHORT", "entry": entry, "setup_type": "📉 BOS 突破"}
    ...
    # 硬性過濾：SL 距離太緊（0.7%）= 噪音等級，放棄
    if risk_pct < SL_MIN_PCT * 100:
        logging.info(f"SL 太緊 ({risk_pct:.2f}%)，跳過")
        return None
...
def main():
    try:
        ...
        if is_midnight or manual_report:
            if not os.path.exists("midnight.ok") or manual_report:
                ...
                if not df_s.empty:
                    ...
                    ev   = (wr/100 * 1.5 + (1-wr/100) * (-1.0)) if total > 0 else 0  # avg TP ~1.5R
        ...
        if daily_limit_hit:
            logging.info(f"今日已止損 {today_sl_count} 次，停止接受新訊號")
        ...
        logging.info(f"BTC 15m:{btc_trend}  BTC 4H:{'📈' if btc_st_4h==1 else('📉' if btc_st_4h==-1 else '⚪')}  時段:{session_label}  今日SL:{today_sl_count}/{MAX_DAILY_SL}")
        ...
        for instId in ALL_COINS:
            ...
            if instId not in active_ids:
                ...
                if instId != "BTC-USDT-SWAP":
                    if setup['side']=="LONG"  and btc_trend=="DOWN":
                        logging.info(f"[{instId}] BTC 15m 下跌跳過"); time.sleep(0.3); continue
                    if setup['side']=="SHORT" and btc_trend=="UP":
                        logging.info(f"[{instId}] BTC 15m 上漲跳過"); time.sleep(0.3); continue

                # 15m Supertrend
                if setup['st_val']==-1 and setup['side']=="LONG":
                    logging.info(f"[{instId}] 15m ST 空頭跳過"); time.sleep(0.3); continue
                if setup['st_val']== 1 and setup['side']=="SHORT":
                    logging.info(f"[{instId}] 15m ST 多頭跳過"); time.sleep(0.3); continue

                # EMA50
                if not check_ema_trend(df, setup['side']):
                    logging.info(f"[{instId}] EMA50 跳過"); time.sleep(0.3); continue
                ...
                if not ok_1h:
                    logging.info(f"[{instId}] 1H ST 跳過"); time.sleep(0.3); continue

                # 4H 大趨勢（只阻止強烈反向）
                ok_4h, label_4h = check_4h_trend(instId, setup['side'])
                if not ok_4h:
                    logging.info(f"[{instId}] 4H ST 跳過 ({label_4h})"); time.sleep(0.3); continue

                # BTC 4H（只阻止強烈反向）
                if instId != "BTC-USDT-SWAP":
                    ok_btc4h, label_btc4h = check_btc_4h_trend(btc_df_4h, setup['side'])
                    if not ok_btc4h:
                        logging.info(f"[{instId}] BTC 4H 跳過"); time.sleep(0.3); continue
                else:
                    label_btc4h = "BTC 自身"
                ...
                if open_count >= MAX_CONCURRENT:
                    logging.info(f"[{instId}] 倉位上限 {MAX_CONCURRENT}，跳過"); time.sleep(0.3); continue
                ...
                msg += f"💎 幣種：#{coin_sym}\n"
                msg += f"🎯 方向：{side_zh}\n"
                msg += f"📐 形態：{setup['setup_type']}\n"
                msg += f"⏰ 週期：15m  |  {session_label}\n"
                msg += f"📊 多空比 {ls_ratio} | 資費 {funding}\n"
                msg += f"{setup['cvd_label']}\n"
                msg += f"\n"
                msg += f"📍 進場位：{setup['entry']:.4f}\n"
                msg += f"🚫 止損位：{setup['sl']:.4f}  (-1R, {setup['risk_pct']:.1f}%)\n"
                msg += f"💰 TP1 (0.7R)：{setup['tp1']:.4f}  ← 觸及後立即保本\n"
                msg += f"💰 TP2 (1.5R)：{setup['tp2']:.4f}\n"
                msg += f"💰 TP3 (3.0R)：{setup['tp3']:.4f}\n"
                msg += f"\n"
                msg += f"🏗️ 結構：{setup['structure']}\n"
                msg += f"📡 ST：{setup['st_label']} | {label_1h} | {label_4h}\n"
                msg += f"₿ BTC 4H：{label_btc4h}\n"
                msg += f"📈 {setup['ema_label']} | RSI {setup['rsi_val']:.1f}\n"
            ...
            if t['status'] == "WAITING":
                ...
                if is_hit:
                    # ⭐ 核心升級：等待拒絕確認棒
                    # 價格觸及進場位後，必須有一根 K 棒「觸及進場位但收盤確認方向」
                    # 這個確認代表市場在該水平有真實支撐/阻力，而非假突破。
                    # 沒有確認棒的觸及 = 可能只是穿過去，大概率止損
                    reject_confirmed = False
                    check_n = min(5, len(df))
                    for idx in range(-check_n, 0):
                        ck = df.iloc[idx]
                        if t['side'] == "LONG":
                            # 觸及進場位（或更低）且收盤在進場位上方
                            if ck['l'] <= t['entry'] * 1.001 and ck['c'] > t['entry']:
                                reject_confirmed = True
                                break
                        else:
                            # 觸及進場位（或更高）且收盤在進場位下方
                            if ck['h'] >= t['entry'] * 0.999 and ck['c'] < t['entry']:
                                reject_confirmed = True
                                break

                    if not reject_confirmed:
                        logging.info(f"[{instId}] 已觸及進場位，等待拒絕確認棒收盤...")
                        updated.append(t)   # 保持 WAITING
                        time.sleep(0.3)
                        continue
                    ...
                    send_tg(
                        ...
                        f"📍 成交價：{t['entry']:.4f}\n"
                        f"🚫 止損位：{t['sl']:.4f}\n"
                        f"💰 TP1 (0.7R)：{t['tp1']:.4f}  ← 觸及後立即移 SL 到成本\n"
                        f"💰 TP2 (1.5R)：{t['tp2']:.4f}\n"
                        f"💰 TP3 (3.0R)：{t['tp3']:.4f}\n"
                        f"\n"
                        f"🎯 *已確認，緊盯止損*"
                ...
                if t['tp1_hit'] == 0 and (
                    ...
                    t['tp1_hit'] = 1
                    t['locked']  = 1          # ⭐ 立即標記保本
                    t['sl']      = t['entry'] # ⭐ 立即移 SL 到成本
                    send_tg(
                        f"🎯 *Alpha Oracle | TP1 達標*\n"
                        f"──────────────────\n"
                        f"💎 幣種：#{coin_sym}\n"
                        f"✅ TP1 (0.7R) 已觸及\n"
                ...
                # TP2 (1.5R) 觸及 → 升級鎖利到 TP1
                if t['locked'] == 1 and (
                    (t['side']=="LONG"  and act_high >= t['tp2']) or
                    (t['side']=="SHORT" and act_low  <= t['tp2'])
                ):
                    t['locked'] = 2
                    t['sl']     = t['tp1']    # SL 升級到 TP1
                    send_tg(
                        f"🔒 *Alpha Oracle | TP2 達標 · 鎖利*\n"
                        f"──────────────────\n"
                        f"💎 幣種：#{coin_sym}\n"
                        f"✅ TP2 (1.5R) 已觸及\n"
                        f"🔒 止損已升級至 TP1：{t['tp1']:.4f}\n"
                        f"\n"
                        f"📍 當前價：{curr_p:.4f}\n"
                        f"💰 TP3 (3.0R)：{t['tp3']:.4f}\n"
                        f"\n"
                ...
                if is_sl or is_tp3:
                    if is_tp3:
                        rl, ep, res = "💰 滿倉止盈 (TP3=3R)", t['tp3'], "TP"
                    elif t['locked'] >= 2:
                        rl, ep, res = "🔒 鎖利出場 (TP2 secured)", t['tp1'], "TP"
                    elif t['locked'] == 1:
                        rl, ep, res = "⚖️ 保本出場 (Break Even)", t['entry'], "TP"
                    else:
                        rl, ep, res = "❌ 止損離場", t['sl'], "SL"
                    ...
                    cooldown_dur = COOLDOWN_BARS_SL if res == "SL" else COOLDOWN_BARS_TP
                    updated.append({**t, "status": "COOLDOWN", "wait_since": current_bar, "locked": cooldown_dur})
                    time.sleep(0.3)
                    continue
                ...
                if bars_cooled >= cooldown_total:
                    logging.info(f"[{instId}] 冷卻結束（{bars_cooled}/{cooldown_total} 棒），恢復掃描")
