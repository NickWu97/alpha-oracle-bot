...
# -*- coding: utf-8 -*-
"""
Alpha Oracle v9.1.7 — 完整修復版（價格監控 + TP 通知強化）
══════════════════════════════════════════════════════════════════════
v9.1.7 修復：
...
def scan_timeframe(instId: str, tf: str, htf_trend: str, fr: float, ls_f: float, ls_str: str, ob_r: float, oi_sc: float, oi_lb: str, _cache: dict) -> list:
    ...
    for side in ["LONG", "SHORT"]:
        ...
        opp = dict(
            instId=instId, side=side, tf=tf, entry=entry, sl=sl_price, tp1=tp1, tp2=tp2, tp3=tp3,
            price=price, atr=atr, structure=structure, bos_desc=bos_desc, at_ob=at_ob, at_fvg=at_fvg,
            ob_d=ob_d, fvg_d=fvg_d, pd_lb=pd_lb, liq=liq, crossline=cl, as_bool=as_bool, as_d=as_d,
            abs_bool=abs_b, abs_desc=abs_d, cvd_lb=cvd_lb, ls_str=ls_str, ls_lb=ls_lb, fr_lb=fr_lb,
            ob_dir_lb=ob_dir_lb, pa_sc=pa_sc, pa_lb=pa_lb, pa_sigs=pa_sigs, whale_zones=whale_zones,
            htf_trend=htf_trend, st_lb=st_lb, regime=regime, has_rsi=has_rsi, rsi_d=rsi_d, rsi_v=rsi_v,
            t4h_lb=t4h_lb, btc_lb=btc_lb, adx_lb=adx_lb, vol_msg=vol_msg, score=score, grade=grade,
            breakdown=bd, lev="10x~20x" if atr/price<0.015 else "3x~5x", vwap_lb=vwap_lb, oi_lb=oi_lb,
            has_macd=has_macd, macd_d=macd_d, pos_hint=pos_hint, session=get_market_session()
        )
...
def scan_for_opportunity(instId: str) -> list:
    ...
    for tf in SCAN_TIMEFRAMES:
        try:
            opps = scan_timeframe(instId, tf, htf_trend_str, fr, ls_f, ls_str, ob_r, oi_sc, oi_lb, _cache)
...
class WinRateTracker:
    ...
    def _stats(self, trades: list):
        ...
        return {
            ...
            "expectancy": exp, "streak_str": streak_str
        }
