...
# -*- coding: utf-8 -*-
"""
Alpha Oracle Pro v15.5 — 即時 Tick 監控版（繁體中文）
══════════════════════════════════════════════════════════════════════
🚨 v15.5 修了 v15.4 致命延遲問題：

   ❌ v15.4 問題：SL/TP 必須等 15m K 線資料才會檢查，加上 cron 5 分鐘
      跑一次，等於有 4 分鐘 dead time，SL 可能延遲 5~10 分鐘才觸發

   ✅ v15.5 修法：
      1. _quick_check_sl_tp：純看當前 price vs SL/TP，不等 K 線
      2. check_all 入口先做全部 ACTIVE 持倉的 tick check
      3. intensive_monitor 改成 3 秒一輪（v15.4 是 10 秒）
      4. fetch_price 加 force_fresh 選項繞過快取
      5. price_cache TTL 由 5 秒縮到 2 秒（活躍部位時）
      6. 配套：websocket_monitor.py 在本機跑可達 < 1 秒延遲

✨ v15.4 三階段訊號系統 + 逆勢模式 + coin+side 冷卻全保留
✨ v15.3 修正：record_trade 重複、hard_filters、TP/SL 順序等
══════════════════════════════════════════════════════════════════════
"""
import os
import sys
...
DEFAULT_CONFIG: dict = {
    ...
    "entry_zone_pct": {
        "long_favor": 0.006, "long_against": 0.002,
        "short_favor": 0.006, "short_against": 0.002,
    },
    ...
    "setup_watch": {
        ...
        "score_threshold": 55,
        "expire_hours": 6,
        "min_dist_to_zone_pct": 0.005,
        "max_dist_to_zone_pct": 0.05,
        "approach_dist_pct": 0.003,
        "min_impulse_atr": 1.5,
    },

    "reversal_mode": {
        "enabled": True, "max_adx": 18, "score_threshold": 60,
        "snr_distance_pct": 0.01, "tp_r_ratios": [1.2, 2.0, 3.0],
        "sl_atr_extra": 0.3,
    },

    "microtrigger": {
        "enabled": True, "min_bullish_candles": 3,
        "lookback_for_breakout": 5,
    },

    "pending_reminder_minutes": 30,
    "heartbeat": {"enabled": True, "hour_mark": True},

    # ⚡ v15.5 即時監控
    "tick_check": {
        "enabled": True,                  # 入口 tick check
        "price_cache_ttl_active": 2,      # 有活躍部位時，價格快取 2 秒
        "price_cache_ttl_idle": 5,        # 無活躍部位時 5 秒
    },
    "intensive_monitor": {
        "enabled": True,
        "total_seconds": 55,              # 跑滿 1 分鐘
        "interval_seconds": 3,            # 每 3 秒輪一次（v15.4 是 10）
    },

    "capital_management": {
        "capital_per_trade_usd": 100, "max_loss_usd": 20,
        "total_capital_usd": 1000,
        "max_leverage": 50, "min_leverage": 2,
    },
    ...
    "circuit_breaker": {"loss_threshold": 3, "pause_hours": 24},
    ...
    "auto_news_blackout": {
        "nfp": True, "cpi": True,
        "et_window_min_before": 35, "et_window_min_after": 60,
    },
...
_session.headers.update({"User-Agent": "alpha-oracle-pro/15.5"})
...
class GlobalLock:
    ...
    def __enter__(self):
        ...
        try:
            ...
            logging.warning(f"⚠️ 已有實例執行中，本次跳過")
            sys.exit(0)
...
# ═════════════════════════════════════════════════════════
# 4. OKX 數據抓取（v15.5 加 force_fresh）
# ═════════════════════════════════════════════════════════
...
def fetch_price(instId: str, force_fresh: bool = False,
    ...
    """⚡ v15.5 加 force_fresh 跟自訂 cache_ttl"""
    now = time.time()
...
def _signal_buckets(score: int, side: str, detail: dict, funding_rate, coin: str) -> list:
    rsi = (detail or {}).get("rsi_value", 50)
    return [
        _bucket_score(score), _bucket_rsi(rsi, side),
...
# 7. Setup 偵測
# ═════════════════════════════════════════════════════════
def detect_setup(instId: str, df: list, current_price: float,
                 mtf: dict, cfg: dict) -> dict | None:
    sw = cfg.get("setup_watch", {})
    if not sw.get("enabled", True): return None
    threshold = sw.get("score_threshold", 55)
    min_dist = sw.get("min_dist_to_zone_pct", 0.005)
    max_dist = sw.get("max_dist_to_zone_pct", 0.05)
    min_impulse = sw.get("min_impulse_atr", 1.5)
    if len(df) < 30: return None
    atr = calc_atr(df)
    ma20 = sum(c["c"] for c in df[-20:]) / 20
    max_dev = max(abs(c["c"] - ma20) for c in df[-min(30, len(df)):])
    if max_dev < atr * min_impulse:
        return None
    candidates = []
    for side in ("LONG", "SHORT"):
        expect = 1 if side == "LONG" else -1
        h4 = mtf.get("4H", {}).get("supertrend", 0)
        if h4 != expect: continue
        zone = find_order_block(df, side, lookback=40)
        zone_type = "OB"
        if not zone:
            zone = find_fvg(df, side, lookback=40)
            zone_type = "FVG"
        if not zone: continue
        if side == "LONG":
            if current_price <= zone["high"]: continue
            dist = (current_price - zone["high"]) / current_price
        else:
            if current_price >= zone["low"]: continue
            dist = (zone["low"] - current_price) / current_price
        if not (min_dist < dist < max_dist): continue
        score, _, detail = calc_score(df, side, current_price, mtf=mtf)
        if score < threshold: continue
        candidates.append({
            "side": side, "zone": zone, "zone_type": zone_type,
            "score": score, "detail": detail,
            "dist_to_zone": dist, "mtf_snapshot": mtf,
        })
...
def generate_signal(instId: str, df: list, current_price: float,
                    funding_rate: float | None, cfg: dict) -> dict | None:
    if df is None or len(df) < 50: return None
    atr = calc_atr(df)
    atr_pct = atr / current_price * 100
    if atr / current_price > cfg.get("atr_max_pct", 0.035): return None
    coin = instId.split("-")[0]
    mtf = fetch_mtf_trend(instId)
    ...
    if min_vol > 0:
        ...
        if vol_ratio < min_vol: return None
    tp_r = cfg.get("tp_r_ratios", [1.5, 3.0, 5.0])
    if len(tp_r) != 3: tp_r = [1.5, 3.0, 5.0]
    ...
    for side in ("LONG", "SHORT"):
        ...
        risk = abs(entry - sl)
        if risk <= 0: continue
        if abs(tp1 - entry) / risk < min_rr - 0.02: continue
        if side == "LONG":
            if not (entry < tp1 < tp2 < tp3): continue
        else:
            if not (entry > tp1 > tp2 > tp3): continue
        ob_zone = find_order_block(df, side)
        grade = ("🔥 A+ 極強" if score >= 90 else
                 "⭐ A 強力" if score >= 80 else "✅ B+ 合格")
        candidates.append({
            "instId": instId, "side": side, "tf": "15m",
            "entry": round(entry, 6), "sl": round(sl, 6),
            "tp1": round(tp1, 6), "tp2": round(tp2, 6), "tp3": round(tp3, 6),
            "score": int(score), "raw_score": int(raw_score), "grade": grade,
            "detail": detail, "funding_rate": funding_rate,
            "mtf_snapshot": mtf, "ob_zone": ob_zone,
            "tp_r_ratios": tp_r, "mode": "trend",
            "created": time.time(),
...
def generate_reversal_signal(instId: str, df: list, current_price: float,
                             funding_rate: float | None, cfg: dict) -> dict | None:
    rm = cfg.get("reversal_mode", {})
    if not rm.get("enabled", True): return None
    if len(df) < 50: return None
    max_adx = rm.get("max_adx", 18)
    if calc_adx(df) >= max_adx: return None
    threshold = rm.get("score_threshold", 60)
    snr_dist = rm.get("snr_distance_pct", 0.01)
    tp_r = rm.get("tp_r_ratios", [1.2, 2.0, 3.0])
    sl_mult = cfg.get("sl_atr_mult", 1.5)
    sl_extra = rm.get("sl_atr_extra", 0.3)
    min_rr = cfg.get("min_rr_ratio", 1.5)
    expire_h = cfg.get("signal_expire_hours", 24)
    sup, res = calc_snr(df, lookback=80)
    mid = (sup + res) / 2
    atr = calc_atr(df)
    atr_pct = atr / current_price * 100
    if atr / current_price > cfg.get("atr_max_pct", 0.035): return None
    candidates = []
    if (current_price <= sup * (1 + snr_dist)
            and detect_liquidity_sweep(df[-15:], "LONG")
            and detect_price_action(df, "LONG")):
        sweep_low = min(c["l"] for c in df[-5:])
        entry = current_price
        sl = min(sweep_low - atr * sl_extra, sup - atr * sl_mult * 0.5)
        risk = entry - sl
        if risk > 0:
            tp1 = entry + risk * tp_r[0]
            tp2 = min(mid, entry + risk * tp_r[1])
            tp3 = min(res * 0.998, entry + risk * tp_r[2])
            if entry < tp1 < tp2 < tp3 and abs(tp1 - entry) / risk >= min_rr - 0.02:
                raw_score, _, detail = calc_score(df, "LONG", current_price)
                detail["atr_pct"] = round(atr_pct, 3)
                if raw_score >= threshold:
                    candidates.append({
                        "instId": instId, "side": "LONG", "tf": "15m",
                        "entry": round(entry, 6), "sl": round(sl, 6),
                        "tp1": round(tp1, 6), "tp2": round(tp2, 6), "tp3": round(tp3, 6),
                        "score": int(raw_score), "raw_score": int(raw_score),
                        "grade": "🔄 逆勢", "detail": detail,
                        "funding_rate": funding_rate,
                        "mtf_snapshot": fetch_mtf_trend(instId),
                        "tp_r_ratios": tp_r, "mode": "reversal",
                        "created": time.time(),
                        "expires": time.time() + expire_h * 3600,
                    })
    if (current_price >= res * (1 - snr_dist)
            and detect_liquidity_sweep(df[-15:], "SHORT")
            and detect_price_action(df, "SHORT")):
        sweep_high = max(c["h"] for c in df[-5:])
        entry = current_price
        sl = max(sweep_high + atr * sl_extra, res + atr * sl_mult * 0.5)
        risk = sl - entry
        if risk > 0:
            tp1 = entry - risk * tp_r[0]
            tp2 = max(mid, entry - risk * tp_r[1])
            tp3 = max(sup * 1.002, entry - risk * tp_r[2])
            if entry > tp1 > tp2 > tp3 and abs(tp1 - entry) / risk >= min_rr - 0.02:
                raw_score, _, detail = calc_score(df, "SHORT", current_price)
                detail["atr_pct"] = round(atr_pct, 3)
                if raw_score >= threshold:
                    candidates.append({
                        "instId": instId, "side": "SHORT", "tf": "15m",
                        "entry": round(entry, 6), "sl": round(sl, 6),
                        "tp1": round(tp1, 6), "tp2": round(tp2, 6), "tp3": round(tp3, 6),
                        "score": int(raw_score), "raw_score": int(raw_score),
                        "grade": "🔄 逆勢", "detail": detail,
...
def check_1m_microtrigger(sig: dict, cfg: dict) -> bool:
    mt = cfg.get("microtrigger", {})
    if not mt.get("enabled", True): return False
    if sig.get("status") != "PENDING": return False
    min_bull = mt.get("min_bullish_candles", 3)
    look = mt.get("lookback_for_breakout", 5)
    df_1m = fetch_candles(sig["instId"], tf="1m", limit=15)
    if len(df_1m) < min_bull + look: return False
    side = sig["side"]
    last_n = df_1m[-min_bull:]
    prev = df_1m[-(min_bull + look):-min_bull]
    if side == "LONG":
        bullish = all(c["c"] > c["o"] for c in last_n)
        rising = all(last_n[i]["c"] <= last_n[i + 1]["c"] for i in range(len(last_n) - 1))
        breakout = bool(prev) and last_n[-1]["c"] > max(c["h"] for c in prev)
        return bullish and rising and breakout
    bearish = all(c["c"] < c["o"] for c in last_n)
    falling = all(last_n[i]["c"] >= last_n[i + 1]["c"] for i in range(len(last_n) - 1))
    breakdown = bool(prev) and last_n[-1]["c"] < min(c["l"] for c in prev)
    return bearish and falling and breakdown
...
def calc_realized_usd(sig: dict, close_type: str) -> float:
    ...
    if not sizing: return 0.0
    tp1_p, tp2_p, tp3_p = sizing["tp1_profit"], sizing["tp2_profit"], sizing["tp3_profit"]
    sl_l = -sizing["sl_loss"]
    if close_type == "TP3": return round((tp1_p + tp2_p + tp3_p) / 3, 2)
    if close_type == "LOCK": return round((tp1_p + tp2_p + tp1_p) / 3, 2)
    if close_type == "BE": return round(tp1_p / 3, 2)
    return round(sl_l, 2)


def calc_realized_r(close_type: str, tp_r: list) -> float:
    if not tp_r or len(tp_r) != 3: tp_r = [1.5, 3.0, 5.0]
    if close_type == "TP3": return round((tp_r[0] + tp_r[1] + tp_r[2]) / 3, 2)
    if close_type == "LOCK": return round((tp_r[0] + tp_r[1] + tp_r[0]) / 3, 2)
    if close_type == "BE": return round(tp_r[0] / 3, 2)
    return -1.0
...
def _fmt_entry(sig: dict, current_price: float) -> str:
    ...
    mode_tag = "🔄 逆勢" if mode == "reversal" else "📈 順勢"
    grade = sig.get("grade", "")
    ...
    return (
        f"{emoji} *{coin} 進場提醒* {grade} ({mode_tag})\n"
        f"━━━━━━━━━━━━━━\n"
        f"🆔 `{sig['order_id']}`\n"
        f"⏰ {tw_ts()}\n"
...
def _fmt_setup(setup_data: dict, current_price: float) -> str:
    coin = setup_data["instId"].split("-")[0]
    side = setup_data["side"]
    direction = "做多" if side == "LONG" else "做空"
    score = setup_data["score"]
    zone = setup_data["zone"]
    zone_type = setup_data.get("zone_type", "OB")
    dist = setup_data["dist_to_zone"] * 100
    return (
        f"📡 *{coin} {direction} Setup 形成* (score {score})\n"
        f"━━━━━━━━━━━━━━\n"
        f"📍 階段 1/3：等待回撤進場\n"
        f"當前 `{current_price:.4f}`\n"
        f"{zone_type} 進場區 `{zone['low']:.4f} ~ {zone['high']:.4f}`\n"
        f"距 zone `{dist:.2f}%`"
    )


def _fmt_approach(setup_data: dict, current_price: float) -> str:
    coin = setup_data["instId"].split("-")[0]
    side = setup_data["side"]
    direction = "做多" if side == "LONG" else "做空"
    zone = setup_data["zone"]
    return (
        f"⚠️ *{coin} {direction}進場區接近*\n"
        f"━━━━━━━━━━━━━━\n"
        f"📍 階段 2/3：盯盤，Trigger 即將出現\n"
        f"當前 `{current_price:.4f}`\n"
        f"進場區 `{zone['low']:.4f} ~ {zone['high']:.4f}`"
    )


def _fmt_pending_reminder(sig: dict, current_price: float) -> str:
    coin = sig["instId"].split("-")[0]
    side = sig["side"]
    entry = sig["entry"]
    dist = abs(current_price - entry) / current_price * 100
    remaining_h = (sig["expires"] - time.time()) / 3600
    return (
        f"📍 *{coin} {side} 仍在等待*\n"
        f"🆔 `{sig['order_id'][-8:]}`\n"
        f"進場 `{entry:.4f}` / 當前 `{current_price:.4f}` (距 `{dist:.2f}%`)\n"
        f"剩餘 `{remaining_h:.1f}h`"
    )
...
def _fmt_final_close(coin: str, order_id: str, close_type: str,
    ...
    w = " 🪡" if wick else ""
    if close_type == "TP3":
        return (f"🏆 *{coin} TP3 全部達標* `{pnl_pct:+.2f}%` (`{realized_r:.1f}R` / `+${pnl_usd:.0f}`){w}\n"
                f"🆔 `{order_id[-8:]}` · {tw_now().strftime('%H:%M')}")
    if close_type == "LOCK":
        return (f"🔐 *{coin} 鎖利出場* `{pnl_pct:+.2f}%` (`{realized_r:.1f}R` / `+${pnl_usd:.0f}`){w}\n"
                f"🆔 `{order_id[-8:]}` · {tw_now().strftime('%H:%M')}")
    if close_type == "BE":
        sign = "+" if pnl_usd >= 0 else ""
        return (f"🔒 *{coin} 保本出場* `{pnl_pct:+.2f}%` (`{realized_r:.1f}R` / `{sign}${pnl_usd:.0f}`){w}\n"
                f"🆔 `{order_id[-8:]}` · {tw_now().strftime('%H:%M')}")
    return (f"❌ *{coin} 止損* `{pnl_pct:+.2f}%` (`{realized_r:.1f}R` / `-${abs(pnl_usd):.0f}`){w}\n"
            f"🆔 `{order_id[-8:]}` · {tw_now().strftime('%H:%M')}")
...
def analyze_loss(sig: dict, df_at_loss: list) -> list:
    ...
    if atr_then > 0 and atr_now / atr_then > 1.5:
        reasons.append({"code": "VOL_SPIKE", "title": "🌪 波動率激增",
                        ...
                        "severity": 18})
    last10 = df_now[-10:]
    against = sum(1 for c in last10
                  if (side == "LONG" and c["c"] < c["o"])
                  or (side == "SHORT" and c["c"] > c["o"]))
    if against >= 7:
        reasons.append({"code": "AGAINST_K", "title": "💪 持續反向動能",
...
def _calc_postmortem_context(sig: dict, df_at_loss: list) -> dict:
    if not df_at_loss or len(df_at_loss) < 20: return {}
    n = len(df_at_loss)
    df_then = df_at_loss[: max(20, n // 3)]
    df_now = df_at_loss
    ...
    return {
        ...
        "rsi_change": calc_rsi(df_now) - calc_rsi(df_then),
        "atr_pct_then": calc_atr(df_then) / sig["entry"] * 100 if sig["entry"] > 0 else 0,
        "atr_pct_now": calc_atr(df_now) / sig["entry"] * 100 if sig["entry"] > 0 else 0,
        "atr_change_pct": (calc_atr(df_now) / calc_atr(df_then) - 1) * 100 if calc_atr(df_then) > 0 else 0,
...
def _generate_lessons(reasons: list) -> list:
    ...
    for r in reasons[:2]:
        ...
        if c and c not in seen and c in advice:
            seen.add(c); out.append(advice[c])
    return out
...
def _fmt_postmortem(sig: dict, close_type: str, reasons: list, lessons: list,
    ...
    if close_type == "BE": result_emoji, result_text = "🔒", "保本"
    elif close_type == "LOCK": result_emoji, result_text = "🔐", "鎖利"
    elif close_type == "TP3": result_emoji, result_text = "🏆", "全勝"
    else: result_emoji, result_text = "❌", "止損"
    ...
    lines = [
        ...
        f"━━━━━━━━━━━━━━",
        f"{result_emoji} 結算 *{result_text}* `{realized_r:+.1f}R` / `{sign}${realized_usd:.0f}`",
        f"📊 開單評分 `{score}` 分 · 持倉 `{duration}`",
        f"",
        f"🎯 *出場主因：*",
    ]
    ...
    if similar and similar[0] >= 3:
        ...
        lines.append("")
        lines.append(f"📚 *同類歷史：* {n} 筆，勝率 `{wr:.0f}%` {verdict}")
    return "\n".join(lines)
...
def check_circuit_breaker(cfg: dict) -> tuple[bool, str]:
    ...
    history = _load_json(TRADE_HISTORY_FILE, [])
    closed = [t for t in history if t.get("close_type") in ("SL", "BE", "LOCK", "TP3")]
    if len(closed) < threshold: return False, ""
    last_n = closed[-threshold:]
    if not all(t.get("close_type") == "SL" for t in last_n): return False, ""
    try:
...
def is_in_news_window(cfg: dict) -> tuple[bool, str]:
    ...
    if not (auto.get("nfp", True) or auto.get("cpi", True)): return False, ""
    if ET_TZ is None: return False, ""
    ...
    if auto.get("nfp", True) and et_now.weekday() == 4 and et_now.day <= 7:
        ...
        if (target - timedelta(minutes=before_min)) <= et_now <= (target + timedelta(minutes=after_min)):
            return True, f"NFP 非農（ET 8:30）"
    if auto.get("cpi", True) and 10 <= et_now.day <= 16:
        target = et_now.replace(hour=8, minute=30, second=0, microsecond=0)
        if (target - timedelta(minutes=before_min)) <= et_now <= (target + timedelta(minutes=after_min)):
            return True, f"CPI 數據（ET 8:30）"
    return False, ""
...
def is_blackout_time(cfg: dict) -> tuple[bool, str]:
    ...
    for w in cfg.get("blackout_windows_tw", []):
        try:
            ...
            in_window = (sm_t <= cur < em_t if sm_t <= em_t
...
def check_daily_limits(cfg: dict, tracker) -> tuple[bool, str]:
    ...
    open_count = sum(1 for s in tracker.signals.values()
    ...
    if loss_limit_usd is not None and stats["pnl_usd"] < -abs(loss_limit_usd):
        return True, f"⚠️ 當日 PnL `${stats['pnl_usd']:.0f}` 已過 -${abs(loss_limit_usd)} 紅線"
    loss_limit_pct = dl.get("daily_loss_limit_pct", 5.0)
    if stats["pnl_pct"] < -loss_limit_pct:
        return True, f"⚠️ 當日 PnL `{stats['pnl_pct']:.2f}%` 已過 -{loss_limit_pct}% 紅線"
    return False, ""


def is_cooling(instId: str, side: str | None = None, hours: float = 3) -> bool:
    cfg = load_config()
    per_side = cfg.get("cooldown_per_side", True)
    cd = _load_json(SIGNAL_COOLDOWN_FILE, {})
    key = f"{instId}_{side}" if (per_side and side) else instId
    last = cd.get(key)
    return last is not None and (time.time() - float(last)) < hours * 3600


def mark_cooldown(instId: str, side: str | None = None, hours: float = 3) -> None:
    cfg = load_config()
    per_side = cfg.get("cooldown_per_side", True)
    cd = _load_json(SIGNAL_COOLDOWN_FILE, {})
    key = f"{instId}_{side}" if (per_side and side) else instId
...
def check_health() -> tuple[bool, str]:
    ...
    if time.time() - last_warn < 6 * 3600: return False, ""
    if last_tg > 0:
        hours = (time.time() - last_tg) / 3600
        if hours > 24:
            state["last_health_warning"] = time.time()
            set_system_state(state)
            return True, f"⚠️ *系統健康警報*\n超過 *{hours:.0f} 小時*沒送過 TG"
    return False, ""


def send_heartbeat(tracker, setup_tracker) -> None:
    cfg = load_config()
    hb = cfg.get("heartbeat", {})
    if not hb.get("enabled", True): return
    if not hb.get("hour_mark", True): return
    now = tw_now()
    if now.minute != 0: return
    state = get_system_state()
    cur_hour_key = now.strftime("%Y%m%d-%H")
    if state.get("last_heartbeat_hour") == cur_hour_key: return
    active = sum(1 for s in tracker.signals.values()
                 if s.get("status") in ("ACTIVE", "BE", "TRAIL"))
    pending = sum(1 for s in tracker.signals.values()
                  if s.get("status") == "PENDING")
    setups = len([s for s in setup_tracker.setups.values()
                  if s.get("stage") in ("setup", "approach")])
    if active == 0 and pending == 0 and setups == 0:
        msg = f"✅ {now.strftime('%H:%M')} 掃描正常，目前無持倉/setup"
    else:
        parts = []
        if active: parts.append(f"持倉 {active}")
        if pending: parts.append(f"Pending {pending}")
        if setups: parts.append(f"Setup {setups}")
        msg = f"✅ {now.strftime('%H:%M')} 掃描正常 · " + " · ".join(parts)
    send_tg(msg)
    state["last_heartbeat_hour"] = cur_hour_key
    set_system_state(state)
...
def record_trade_final(sig: dict, close_type: str, close_price: float) -> None:
    ...
    pnl_pct = (((avg_close - entry) / entry * 100)
               if side == "LONG"
    ...
    trade = {
        ...
        "pnl": round(pnl_pct, 2), "pnl_usd": realized_usd,
        "realized_r": realized_r,
        "tp_hits": {"tp1": bool(sig.get("hit_tp1")),
                    "tp2": bool(sig.get("hit_tp2")),
                    "tp3": bool(sig.get("hit_tp3"))},
        "is_win": is_win, "score": score, "raw_score": raw_score,
        "mode": sig.get("mode", "trend"),
...
# ═════════════════════════════════════════════════════════
class SetupTracker:
    def __init__(self, filepath: str = SETUPS_FILE):
        self.filepath = filepath
        self.setups: dict = _load_json(filepath, {})

    def _save(self) -> None:
        _save_json(self.filepath, self.setups)

    def has_setup(self, instId: str, side: str) -> bool:
        return any(
            s for s in self.setups.values()
            if s.get("instId") == instId and s.get("side") == side
            and s.get("stage") in ("setup", "approach")
        )

    def add(self, instId: str, setup_data: dict, cfg: dict) -> str:
        sw = cfg.get("setup_watch", {})
        expire_h = sw.get("expire_hours", 6)
        key = f"setup_{instId}_{setup_data['side']}_{int(time.time())}"
        self.setups[key] = {
            **setup_data, "instId": instId,
            "created": time.time(),
            "expires": time.time() + expire_h * 3600,
            "stage": "setup", "approach_sent": False,
            "setup_msg_id": None,
        }
        self._save()
        return key

    def send_setup_alert(self, key: str, current_price: float) -> None:
        setup = self.setups.get(key)
        if not setup: return
        msg_id = send_tg(_fmt_setup(setup, current_price))
        if msg_id:
            setup["setup_msg_id"] = msg_id
            self._save()

    def update_approach(self, cfg: dict) -> None:
        sw = cfg.get("setup_watch", {})
        approach_dist = sw.get("approach_dist_pct", 0.003)
        to_remove = []
        for key, setup in list(self.setups.items()):
            ...
            if side == "LONG":
                ...
                dist = max(0.0, (price - zone["high"]) / price) if price > zone["high"] else 0.0
            else:
                in_zone = zone["low"] <= price <= zone["high"]
                dist = max(0.0, (zone["low"] - price) / price) if price < zone["low"] else 0.0
            if in_zone:
                if not setup.get("approach_sent"):
                    send_tg(
                        f"⚠️ *{instId.split('-')[0]} 已進入進場區*\n"
                        f"當前 `{price:.4f}` / Zone `{zone['low']:.4f} ~ {zone['high']:.4f}`",
                        reply_to_message_id=setup.get("setup_msg_id"),
                    )
                    setup["approach_sent"] = True
                    setup["stage"] = "approach"
                    self._save()
            elif dist <= approach_dist and not setup.get("approach_sent"):
                send_tg(_fmt_approach(setup, price),
                        reply_to_message_id=setup.get("setup_msg_id"))
                setup["approach_sent"] = True
                setup["stage"] = "approach"
                self._save()
    ...
    def consume_on_trigger(self, instId: str, side: str) -> None:
        to_remove = [k for k, s in self.setups.items()
                     if s.get("instId") == instId and s.get("side") == side
                     and s.get("stage") in ("setup", "approach")]
        for k in to_remove:
            del self.setups[k]
...
# ═════════════════════════════════════════════════════════
# 14. ⚡ v15.5 SignalTracker（核心 tick check 在這裡）
# ═════════════════════════════════════════════════════════
class SignalTracker:
    ...
    def reload(self) -> None:
        """重新從磁碟讀取（給 websocket monitor 用）"""
        self.signals = _load_json(self.filepath, {})
    ...
    def add(self, signal: dict, active: bool = False) -> tuple[str, str]:
        ...
        self.signals[key] = {
            **signal, "order_id": order_id,
            ...
            "last_pending_reminder_ts": now_ts,
        }
        self._save()
        logging.info(f"📌 新增：{order_id} ({signal['instId']} {signal['side']})")
        return key, order_id

    def has_open_position(self, instId: str, side: str | None = None) -> bool:
        for s in self.signals.values():
            if s.get("instId") != instId: continue
            if s.get("status") not in ("PENDING", "ACTIVE", "BE", "TRAIL"): continue
            if side is None or s.get("side") == side: return True
        return False
    ...
    # ═════════════════════════════════════════════════════
    # ⚡ v15.5 CORE：tick-based 即時 SL/TP 檢查
    # ═════════════════════════════════════════════════════
    def quick_tick_check(self, force_fresh: bool = True) -> list:
        """⚡ 對所有 ACTIVE 持倉做純 tick check（不等 K 線）

        這是 v15.5 解決「SL 已破但沒平倉」的核心：
        - 為每個活躍部位抓最新即時價
        - 用 synthetic candle (h=l=c=price) 跑 _process_candle
        - SL/TP 一觸到立刻觸發
        """
        if not self.cfg.get("tick_check", {}).get("enabled", True):
            return []
        ...
        for key, sig in list(self.signals.items()):
            ...
            if price <= 0:
                continue
            # 構造 synthetic candle：h=l=c=current price
            synth_candle = {
            ...
            try:
                if self._process_candle(sig, synth_candle):
                    to_remove.append(key)
                    logging.info(f"⚡ tick check 觸發出場：{sig['instId']} {sig['side']} @ {price}")
            except Exception as e:
    ...
    def check_all(self) -> None:
        self.transitions = 0
        # ⚡ Step 1: tick check（即時）
        removed_by_tick = self.quick_tick_check(force_fresh=True)

        # Step 2: K-line based（為了 wick 判斷與 PENDING 處理）
        to_remove = []
        for key, sig in list(self.signals.items()):
    ...
    def _check_pending(self, sig: dict, price: float) -> bool:
        ...
        micro_trigger = (not in_zone) and check_1m_microtrigger(sig, self.cfg)
        if in_zone or micro_trigger:
            now_ts = time.time()
            sig["status"] = "ACTIVE"
            sig["activated_at"] = now_ts
            sig["last_checked_ts"] = now_ts
            trigger_note = "\n⚡ 1m 微結構提前觸發" if micro_trigger else ""
            msg_id = send_tg(_fmt_entry(sig, price) + trigger_note,
            ...
            self.transitions += 1
            return False
        reminder_min = self.cfg.get("pending_reminder_minutes", 30)
        last_rem = sig.get("last_pending_reminder_ts", sig.get("created", 0))
        if (time.time() - last_rem) > reminder_min * 60:
            send_tg(_fmt_pending_reminder(sig, price),
                    reply_markup=_order_keyboard(order_id))
            sig["last_pending_reminder_ts"] = time.time()
            self._save()
        return False
    ...
    def _process_candle(self, sig: dict, candle: dict) -> bool:
        ...
        if not sig.get("hit_tp1") and favor_hit(tp1):
            ...
            tp_r = sig.get("tp_r_ratios", [1.5, 3.0, 5.0])
            send_tg(_fmt_tp_milestone(coin, order_id, "TP1", tp1, pnl, tp_r[0],
        ...
        if not sig.get("hit_tp2") and favor_hit(tp2):
            ...
            tp_r = sig.get("tp_r_ratios", [1.5, 3.0, 5.0])
            send_tg(_fmt_tp_milestone(coin, order_id, "TP2", tp2, pnl, tp_r[1],
    ...
    def _finalize_close(self, sig: dict, exit_price: float, candle: dict,
        ...
        reply_to = sig.get("entry_message_id")
        kb = _order_keyboard(order_id)
        if sig.get("hit_tp3"): close_type = "TP3"
        elif sig.get("hit_tp2"): close_type = "LOCK"
        elif sig.get("hit_tp1"): close_type = "BE"
        else: close_type = "SL"
        if close_type == "TP3":
            avg_close = (sig["tp1"] + sig["tp2"] + sig["tp3"]) / 3
        elif close_type == "LOCK":
        ...
        realized_r = calc_realized_r(close_type, tp_r)
        realized_usd = calc_realized_usd(sig, close_type)
        send_tg(_fmt_final_close(coin, order_id, close_type, exit_price,
                                  pnl_pct, realized_usd, realized_r, wick),
                reply_markup=kb, reply_to_message_id=reply_to)
        record_trade_final(sig, close_type, exit_price)
        if close_type != "TP3":
            self._send_postmortem(sig, close_type)
        elif (self.cfg.get("post_mortem", {}).get("enabled", True)
              and not self.cfg.get("post_mortem", {}).get("loss_only", False)):
            self._send_postmortem(sig, close_type)
        self.transitions += 1
        return True
    ...
    def _send_postmortem(self, sig: dict, close_type: str) -> None:
        ...
        order_id = sig.get("order_id", "?")
        try:
            if not self.cfg.get("post_mortem", {}).get("enabled", True): return
            ...
            send_tg(_fmt_postmortem(sig, close_type, reasons, lessons, similar, context),
                    reply_to_message_id=sig.get("entry_message_id"))
            if close_type == "SL":
    ...
    def send_position_updates(self) -> None:
        state = get_system_state()
        now = time.time()
        if now - state.get("last_position_update_ts", 0) < 15 * 60: return
        cnt = 0
    ...
    def get_position_stats(self) -> str:
        ...
        for i, p in enumerate(positions):
            ...
            pnl_e = "🟢" if pnl >= 0 else "🔴"
            progress = ("🏆 TP3" if p.get("hit_tp3")
                        else "🥈 TP2" if p.get("hit_tp2")
                        else "🥇 TP1" if p.get("hit_tp1")
                        else "⏳")
            mode_tag = " 🔄" if p.get("mode") == "reversal" else ""
            lines.append(
                f"*{coin}*{mode_tag} · {side} · {p.get('score', 0)} 分\n"
...
def format_daily_report(date: str | None = None) -> str:
    if date is None: date = tw_now().strftime("%Y-%m-%d")
    history = _load_json(TRADE_HISTORY_FILE, [])
    today = [t for t in history if t.get("date") == date]
    s = _summarize(today)
    if s["n"] == 0: return f"📭 *日報 {date}*\n當日尚無交易"
    lines = [
        f"📊 *日報 {date}*",
        f"━━━━━━━━━━━━━━",
...
# 16. 主掃描
# ═════════════════════════════════════════════════════════
def run_scan(tracker: SignalTracker, setup_tracker: SetupTracker) -> int:
    cfg = load_config()
    ...
    for instId in coins:
        ...
        try:
            ...
            if not sig:
                sig = generate_reversal_signal(instId, df, okx_price, funding, cfg)
            if sig:
                side = sig["side"]
                if tracker.has_open_position(instId, side): continue
                if is_cooling(instId, side, cooldown_h): continue
                setup_tracker.consume_on_trigger(instId, side)
                ...
                if in_zone:
                    signal_with_id = dict(sig); signal_with_id["order_id"] = order_id
                    msg_id = send_tg(_fmt_entry(signal_with_id, okx_price),
                                     reply_markup=_order_keyboard(order_id))
                    tracker.set_entry_message_id(key, msg_id)
                    logging.info(f"✅ {instId} {side} Trigger（{order_id}）")
                else:
                    send_tg(
                        f"📍 *{instId.split('-')[0]} {side} 訊號就位*\n"
                        f"🆔 `{order_id}`\n"
                        f"進場 `{sig['entry']:.4f}`（當前 `{okx_price:.4f}`）",
                        reply_markup=_order_keyboard(order_id))
                    logging.info(f"📍 {instId} {side} PENDING")
                mark_cooldown(instId, side, cooldown_h)
                sent += 1
                continue
            if cfg.get("setup_watch", {}).get("enabled", True):
                mtf = fetch_mtf_trend(instId)
                setup = detect_setup(instId, df, okx_price, mtf, cfg)
                if setup:
                    side = setup["side"]
                    if not setup_tracker.has_setup(instId, side):
                        key = setup_tracker.add(instId, setup, cfg)
                        setup_tracker.send_setup_alert(key, okx_price)
                        logging.info(f"📡 {instId} {side} Setup")
        except Exception as e:
    ...
    tracker.check_all()
    tracker.send_position_updates()
    logging.info(f"✅ 掃描完成，本輪 Trigger {sent} 筆")

    if tracker.signals:
        intensive_monitor(tracker, setup_tracker)
    return sent


def intensive_monitor(tracker: SignalTracker, setup_tracker: SetupTracker,
                      total_seconds: int | None = None,
                      interval_seconds: int | None = None) -> None:
    """⚡ v15.5：總時長 55 秒、每 3 秒一輪（之前 50 秒/10 秒）"""
    cfg = load_config()
    im = cfg.get("intensive_monitor", {})
    if not im.get("enabled", True): return
    if total_seconds is None:
        total_seconds = im.get("total_seconds", 55)
    if interval_seconds is None:
        interval_seconds = im.get("interval_seconds", 3)
    if not tracker.signals and not setup_tracker.setups: return
    end_time = time.time() + total_seconds
    polls = 0
    while time.time() < end_time:
        time.sleep(interval_seconds)
        try:
            # ⚡ tick check 是最重要的，每輪都做
            tracker.quick_tick_check(force_fresh=True)
            # 加減清快取再走完整流程
            if polls % 3 == 2:
                _price_cache.clear()
                ...
                setup_tracker.update_approach(cfg)
            polls += 1
            if not tracker.signals and not setup_tracker.setups:
                break
        except Exception as e:
            logging.error(f"❌ intensive_monitor: {e}")
    logging.info(f"📡 即時監控：{polls} 輪 × {interval_seconds}s")


def run_monitor(tracker: SignalTracker, setup_tracker: SetupTracker) -> None:
    cfg = load_config()
    setup_tracker.update_approach(cfg)
    tracker.check_all()
    tracker.send_position_updates()
        ...
        with GlobalLock(LOCK_FILE):
            ...
            if len(sys.argv) > 1:
                ...
                if cmd in ("monitor", "/monitor", "/監控"):
                    run_monitor(tracker, setup_tracker); return
                if cmd in ("tick", "/tick"):
                    # ⚡ 純 tick check 模式（給高頻 cron 用，最快）
                    tracker.quick_tick_check(force_fresh=True)
                    return
            run_scan(tracker, setup_tracker)
            logging.info("🎉 完成")
    except SystemExit:
        raise
    except Exception as e:
