...
# -*- coding: utf-8 -*-
"""
Alpha Oracle v9.1.7 — 完整調試版（價格監控 + TP 通知強化）
══════════════════════════════════════════════════════════════════════
v9.1.7 修復：
...
CONFIRM_TP_ON_CLOSE = os.getenv("CONFIRM_TP_ON_CLOSE", "false").lower() == "true"
...
def get_4h_trend(instId: str, side: str, _cache: dict) -> tuple:
    ...
    ema21 = calculate_ema(df4h["c"], 21).iloc[-1]; price = df4h["c"].iloc[-1]
    if side == "LONG":
...
# ─────────────────────────────────────────────────────────
# 6-11. 市場結構、流動性、Order Block、訂單流、情緒、評分（完整實作）
# ─────────────────────────────────────────────────────────
...
def detect_bos_choch(df: pd.DataFrame, side: str) -> tuple:
    ...
    price, atr = df["c"].iloc[-1], calculate_atr(df); result, score = "無明顯結構", 0.0
    if side=="LONG":
...
def find_liquidity_pools(df: pd.DataFrame, side: str, lookback: int = 60) -> dict:
    ...
    price, atr = df["c"].iloc[-1], calculate_atr(df)
...
def format_alert(coin: str, side: str, alert_type: str, price: float, entry: float, sl: float, tp1: float, tp2: float, tp3: float, new_sl: float = None, score: int = 0) -> str:
    ...
    if alert_type == "ENTRY":
        ...
        pnl = ((price - entry) / entry * 100) if side == "LONG" else ((entry - price) / entry * 100); is_be = new_sl is not None and abs(new_sl - entry) < entry * 0.0001; label = "保本止損" if is_be else "止損觸發"; header_em = "🛡" if is_be else "🛑"; sl_display = f"`{new_sl:.4f}`" if new_sl else f"`{sl:.4f}`"; r_tag = "`0.0R`" if is_be else "`-1.0R`"; outcome = "💡 倉位已平倉於成本價 資金安全，等下一個機會 💪" if is_be else "⚠️ 倉位已止損出場 請遵守風控，莫加碼攤平"
        return f"━━━━━━━━━━━━━━━━━━━━━━━━\n{header_em} *{label}* #{coin} {arrow} {st}\n━━━━━━━━━━━━━━━━━━━━━━━━\n\n💹 當前 `{price:.4f}` `{pnl:+.2f}%` {r_tag}\n\n─────────────────────────\n{header_em} 止損價 {sl_display} 已觸發\n─────────────────────────\n\n{outcome}"
    return ""

# ─────────────────────────────────────────────────────────
# 14. WinRateTracker — 勝率統計 & 戰報（完整實作）
# ─────────────────────────────────────────────────────────
...
class SignalTracker:
    ...
    def _is_close_confirmed(self, sig: dict, level: float) -> bool:
        if not CONFIRM_TP_ON_CLOSE: logging.debug(f"  [{sig['instId']}] CONFIRM_TP_ON_CLOSE=False，即時觸發"); return True
        df = fetch_okx_last_closed(sig["instId"], tf=sig["tf"], limit=3)
        if df is None or len(df) < 1: logging.warning(f"  [{sig['instId']}] 收盤確認抓不到 K 線，暫不觸發"); return False
        last_close = float(df["c"].iloc[-1])
        if sig["side"] == "LONG": confirmed = last_close >= level
        else: confirmed = last_close <= level
        logging.debug(f"  [{sig['instId']}] 收盤確認: close={last_close:.4f}, level={level:.4f}, confirmed={confirmed}"); return confirmed
    def check_one(self, key: str, sig: dict) -> bool:
        try:
            ...
            if sl_hit:
                ...
                if send_tg(msg): logging.info(f"  [{close_type}] {key} @ {price:.4f} (BE={is_be}) - 通知已發送")
            ...
            if tp3_hit:
                ...
                if send_tg(msg): logging.info(f"  [TP3] {key} @ {price:.4f} ✅ 完美收割 - 通知已發送")
            ...
            if tp2_touched and not sig.get("hit_tp2"):
                if not sig.get("touched_tp2"): self.update(key, touched_tp2=True); logging.info(f"  [TP2 觸及] {key} @ {price:.4f} 等收盤確認 (CONFIRM={CONFIRM_TP_ON_CLOSE})")
                if not self._is_close_confirmed(sig, tp2): logging.info(f"  [TP2 待確認] {key} 本根 K 尚未收盤於 TP2 之外"); return False
                now = time.time()
                if not sig.get("hit_tp1"): self.update(key, hit_tp1=True, touched_tp1=True, hit_tp1_at=now); self._close(sig, tp1, "TP1")
                self.update(key, hit_tp2=True, sl=tp1, status="TRAIL", hit_tp2_at=now); msg = format_alert(coin, side, "TP2", price, entry, sig["sl_orig"], tp1, tp2, tp3, new_sl=tp1)
                if send_tg(msg): logging.info(f"  [TP2] {key} @ {price:.4f} → SL 移至 TP1={tp1:.4f} - 通知已發送")
            ...
            if tp1_touched and not sig.get("hit_tp1"):
                if not sig.get("touched_tp1"): self.update(key, touched_tp1=True); logging.info(f"  [TP1 觸及] {key} @ {price:.4f} 等收盤確認 (CONFIRM={CONFIRM_TP_ON_CLOSE})")
                if not self._is_close_confirmed(sig, tp1): logging.info(f"  [TP1 待確認] {key} 本根 K 尚未收盤於 TP1 之外"); return False
                self.update(key, hit_tp1=True, sl=entry, status="BE", hit_tp1_at=time.time()); msg = format_alert(coin, side, "TP1", price, entry, sig["sl_orig"], tp1, tp2, tp3, new_sl=entry)
                if send_tg(msg): logging.info(f"  [TP1] {key} @ {price:.4f} → SL 移至保本={entry:.4f} - 通知已發送")
    ...
    def status_summary(self) -> str:
        ...
        lines.extend(["", f"━━━━━━━━━━━━━━━━━━━━━━━━", f"🤖 Alpha Oracle v9.1.7 動態追蹤中"]); return "\n".join(lines)
...
def _check_entry_zone(opp: dict) -> tuple:
    ...
    elif (side=="LONG" and live > entry): return False, live, f"價格高於進場區 {dist_pct:+.2f}% 等待回踩"
    elif (side=="SHORT" and live < entry): return False, live, f"價格低於進場區 {dist_pct:+.2f}% 等待回升"
    else: return False, live, f"等待接近進場區（{abs(dist_pct):.2f}%）"
...
def run_monitor_once(tracker: SignalTracker, push_status: bool = None) -> int:
    ...
    if _DEBUG_MODE or push_status is None: should_push = bool(remaining)
    elif push_status is False: should_push = False
    else: should_push = remaining and (transitions > 0 or utc_now().minute < HEARTBEAT_MINUTE_WINDOW)
    if should_push:
        reason = "調試模式" if _DEBUG_MODE else ("狀態變動" if transitions > 0 else "整點心跳"); logging.info(f"monitor_once: 推播狀態摘要（{reason}）")
        status_msg = tracker.status_summary()
        if send_tg(status_msg): logging.info("✅ 狀態摘要已發送")
        else: logging.error("❌ 狀態摘要發送失敗")
...
def main():
    ...
    logging.info("=" * 60); logging.info("🤖 Alpha Oracle v9.1.7 啟動（調試版）"); logging.info(f"📋 模式: {args.mode}"); logging.info(f"⏱ 監控間隔: {args.interval}秒"); logging.info(f"🎯 TP 收盤確認: {CONFIRM_TP_ON_CLOSE} (env: CONFIRM_TP_ON_CLOSE)"); logging.info(f"🐛 除錯模式: {_DEBUG_MODE} (env: DEBUG_MODE)"); logging.info(f"🔑 TG_TOKEN 設定: {'✅' if TG_TOKEN else '❌'}"); logging.info(f"💬 CHAT_ID 設定: {'✅' if CHAT_ID else '❌'}"); logging.info("=" * 60)
    if TG_TOKEN and CHAT_ID:
        test_msg = "🔧 Alpha Oracle v9.1.7 啟動測試 - 價格監控運作中"
        if send_tg(test_msg): logging.info("✅ Telegram 連線測試成功")
        else: logging.error("❌ Telegram 連線測試失敗！請檢查 TG_TOKEN/CHAT_ID")
