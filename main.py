...
# 1. 系統環境變數 (請確保 GitHub Secrets 已設定 TG_TOKEN 與 CHAT_ID)
TG_TOKEN = os.getenv("TG_TOKEN")
CHAT_ID = os.getenv("CHAT_ID")

# 2. 監控清單 (已修正 ASI，包含主流與熱門山寨)
COINS = [
...
def fetch_analysis(instId, mode="SMC"):
    try:
        base = instId.split('-')[0]
        # SMC 監控用 15m 短線找進場點；早盤報表用 12H 長線定趨勢
        ...
        # --- 邏輯 B: 高勝率 SMC + FVG 進場邏輯 ---
            ...
            if is_choch_bull and df['l'].iloc[-1] > df['h'].iloc[-3]:
                ...
                fvg_entry = (df['h'].iloc[-1] < df['l'].iloc[-3])
            ...
            if (is_choch_bull or is_choch_bear) and fvg_entry:
                if is_choch_bull:
                    side = "🟢 強力看多 (CHoCH + FVG)"
                    sl = last_lo * 0.997 # 止損設在前低下方 0.3%
                    tp1 = curr_p + (curr_p - sl) * 1.5
                    tp2 = curr_p + (curr_p - sl) * 3.0
                else:
                    side = "🔴 強力看空 (CHoCH + FVG)"
                    sl = last_hi * 1.003 # 止損設在前高上方 0.3%
                ...
                return (f"🔥 *SMC 高勝率進場訊號*\n"
                        ...
                        f"💎 幣種：#{base}\n"
                        f"🎯 動作：{side}\n\n"
                        f"📍 建議進場位：`{fvg_entry:.4f}` (回踩掛單)\n"
                        f"🚫 止損位 (SL)：`{sl:.4f}`\n"
                        f"💰 止盈位 (TP1)：`{tp1:.4f}` (1.5R)\n"
                        f"💰 止盈位 (TP2)：`{tp2:.4f}` (3.0R)\n\n"
                        f"💡 策略：結構轉變後，在 FVG 缺口處等待回踩。")
            return None
    except:
        return None
