...
def find_smc_setup(df):
    ...
    # 掃描最近的 K 線找尋 BOS 與 FVG
    for i in range(len(df)-3, len(df)-20, -1):
        k1, k2, k3 = df.iloc[i], df.iloc[i+1], df.iloc[i+2]
        # 多頭：BOS (突破前 20 根高點) + FVG (k3.low > k1.high)
        if k3['c'] > k3['o'] and k3['l'] > k1['h'] and k3['h'] > df.iloc[i-20:i]['h'].max():
            return {"side": "LONG", "entry": k1['h'], "sl": k1['l']}
        # 空頭：BOS (跌破前 20 根低點) + FVG (k3.high < k1.low)
        if k3['c'] < k3['o'] and k3['h'] < k1['l'] and k3['l'] < df.iloc[i-20:i]['l'].min():
    ...
    if now_tw.hour == 0 and 0 <= now_tw.minute < 15:
        if not os.path.exists("midnight_report.ok"):
            ...
            if not df_s.empty:
                ...
                msg = f"📊 *Alpha Oracle | 每日實戰結報*\n──────────────────\n🗓 結算：{(now_tw - timedelta(days=1)).strftime('%Y/%m/%d')}\n✅ 止盈：{tp_c} | ❌ 止損：{sl_c}\n🔥 勝率：*{wr:.1f}%*\n──────────────────"
                send_tg(msg)
                pd.DataFrame(columns=["instId","result","time"]).to_csv(STATS_FILE, index=False)
    ...
    for instId in ALL_COINS:
        ...
        if t['status'] == "WAITING":
            ...
            if is_tp2 and t['locked'] == 0:
                t['locked'] = 1
                send_tg(f"🔒 *鎖定利潤提醒* | #{instId.split('-')[0]}\n⚠️ 已達 **TP2 (2.0R)**，請移損至 **TP1 (`{t['tp1']:.4f}`)**！")
            ...
            if is_sl:
                res = "鎖利離場 (1.5R)" if t['locked'] == 1 else "止損離場"
                send_tg(f"⚠️ *結算：{res}* | #{instId.split('-')[0]}")
                pd.DataFrame([{"instId":instId,"result":"SL","time":now_tw}]).to_csv(STATS_FILE, mode='a', header=False, index=False)
                continue
            if is_tp3:
                send_tg(f"🚀 *結算：TP3 止盈 (3.0R)* | #{instId.split('-')[0]} 💰")
