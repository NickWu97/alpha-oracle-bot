...
def find_smc_setup(df):
    ...
    for i in range(len(df)-3, len(df)-20, -1):
        ...
        # 1. 多頭結構破壞 (Choch/BOS) + 成交量放大
        if k2['c'] > df['h'].iloc[i-15:i].max() and k2['v'] > vol_sma:
            swing_low = df['l'].iloc[i-15:i+1].min()
            swing_high = k2['c']
            equilibrium = (swing_high + swing_low) / 2 # 50% 線
            
            # 優先找 FVG 中軸，但必須在 50% 線(折價區)以下
    ...
    try:
        ...
        for instId in ALL_MONITOR:
            ...
            if t['status'] == "WAITING":
                ...
                if is_hit:
                    t['status'] = "ACTIVE"
                    send_tg(f"🚀 *Alpha Oracle | 成交提醒*\n#{instId.split('-')[0]} 已回踩成交，R值：{t['r_ratio']}")
                ...
                if is_sl or is_tp:
                    ...
                    send_tg(f"🏁 *Alpha Oracle | 結算*\n#{instId.split('-')[0]} 離場，結果：{'獲利/保本' if res=='TP' else '止損'}")
