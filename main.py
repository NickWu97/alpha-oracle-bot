...
# -*- coding: utf-8 -*-
"""
Alpha Oracle v9.1.8 — 完整修復版 (SyntaxError Fixed)
══════════════════════════════════════════════════════════════════════
v9.1.8 修復：
  🐛 FIX: check_ob_direction 等函式的 f-string 未閉合導致 SyntaxError
  ✨ NEW: SMC 自動回報 + K 線 wick fallback + 午夜戰報
══════════════════════════════════════════════════════════════════════
"""
import requests
import os
...
def calculate_adx(df: pd.DataFrame, period: int = 14) -> tuple:
    ...
    plus_di = 100*p_w/(atr_w+1e-10); minus_di = 100*m_w/(atr_w+1e-10)
...
def analyze_vwap_position(df: pd.DataFrame, side: str) -> tuple:
    vwap = calculate_vwap(df); price = df["c"].iloc[-1]; atr = calculate_atr(df)
    if side == "LONG":
...
def calculate_macd(df: pd.DataFrame) -> tuple:
    close = df["c"]; ema_fast = calculate_ema(close, MACD_FAST); ema_slow = calculate_ema(close, MACD_SLOW)
    macd_line = ema_fast - ema_slow; signal = calculate_ema(macd_line, MACD_SIGNAL_PERIOD)
    histogram = macd_line - signal
    return macd_line, signal, histogram

def detect_macd_divergence(df: pd.DataFrame, side: str) -> tuple:
    if len(df) < MACD_SLOW + 20: return False, "MACD 數據不足"
    macd, _, _ = calculate_macd(df); lookback = 20; macd_arr = macd.tail(lookback).values
    price_h = df["h"].tail(lookback).values; price_l = df["l"].tail(lookback).values; mid = lookback // 2
    if side == "LONG":
...
def adx_regime_bonus(regime: dict, side: str) -> tuple:
    adx = regime["adx"]; is_uptrend = regime["trend_dir"] == "上升趨勢"
    if adx >= 25:
...
def detect_rsi_divergence(df: pd.DataFrame, side: str) -> tuple:
    ...
    cur_rsi = float(rsi.iloc[-1]); mid = lookback // 2
    if side == "LONG":
        prev_l, curr_l = price_l[:mid].min(), price_l[mid:].min()
        idx1, idx2 = int(np.argmin(price_l[:mid])), mid + int(np.argmin(price_l[mid:]))
        rsi_1, rsi_2 = rsi_arr[idx1], rsi_arr[idx2]
        if curr_l < prev_l * 0.999 and rsi_2 > rsi_1 + 3.0: return True, f"看漲背離 RSI={cur_rsi:.1f}", cur_rsi
    else:
        prev_h, curr_h = price_h[:mid].max(), price_h[mid:].max()
        idx1, idx2 = int(np.argmax(price_h[:mid])), mid + int(np.argmax(price_h[mid:]))
        rsi_1, rsi_2 = rsi_arr[idx1], rsi_arr[idx2]
        if curr_h > prev_h * 1.001 and rsi_2 < rsi_1 - 3.0: return True, f"看跌背離 RSI={cur_rsi:.1f}", cur_rsi
...
def get_4h_trend(instId: str, side: str, _cache: dict) -> tuple:
    ...
    st4, _ = calculate_supertrend(df4h); ema21 = calculate_ema(df4h["c"], 21).iloc[-1]; price = df4h["c"].iloc[-1]
    if side == "LONG":
...
def check_extreme_volatility(df: pd.DataFrame) -> tuple:
    ...
    if ratio > VOLATILITY_HARD_LIMIT: return False, f"極端波動 ATR={ratio*100:.2f}%"
...
def detect_bos_choch(df: pd.DataFrame, side: str) -> tuple:
    sh, sl, _, _ = find_swing_points(df, n=3, lookback=80); price = df["c"].iloc[-1]; atr = calculate_atr(df)
...
def detect_market_structure(df: pd.DataFrame, side: str) -> str:
    ...
    recent = df.tail(20); slope = (recent["c"].iloc[-1]-recent["c"].iloc[0])/(recent["c"].iloc[0]+1e-10)
...
def find_liquidity_pools(df: pd.DataFrame, side: str, lookback: int = 60) -> dict:
    sh, sl, _, _ = find_swing_points(df, n=2, lookback=lookback); price = df["c"].iloc[-1]; atr = calculate_atr(df)
...
def check_ob_fvg_entry(df: pd.DataFrame, side: str, atr: float) -> tuple:
    ...
    at_ob = False; at_fvg = False; ob_d = "無 OB"; fvg_d = "無 FVG"; ez = price
    for ob in obs:
...
# ✅ 修復處：check_ob_direction 確保所有 f-string 正確閉合
...
def scan_timeframe(instId: str, tf: str, htf_trend: str, fr: float, ls_f: float, ls_str: str, ob_r: float, oi_sc: float, oi_lb: str, _cache: dict) -> list:
    ...
    for side in ["LONG", "SHORT"]:
        ...
        price = df["c"].iloc[-1]; sh, sl, _, _ = find_swing_points(df, n=2, lookback=30)
        ...
        if deviation > max(atr_ratio * 0.8, 0.008): logging.info(f"  [略過/{instId}/{tf}/{side}] 即時價 {live:.4f} 偏離進場 {entry:.4f} 過遠 ({deviation*100:.2f}%)"); continue
...
def format_signal(opp: dict) -> str:
    coin = opp["instId"].split("-")[0]; is_long = opp["side"] == "LONG"; arrow = "🟢" if is_long else "🔴"; dir_txt = "LONG" if is_long else "SHORT"; sign = "+" if is_long else "-"; sl_sign = "-" if is_long else "+"
    htf_e = {"LONG":"🟢","SHORT":"🔴","NEUTRAL":"⚪","UNKNOWN":"⚪"}.get(opp["htf_trend"], "⚪"); entry = opp["entry"]
    sl_pct = abs(entry - opp["sl"]) / entry * 100; tp1_pct = abs(opp["tp1"] - entry) / entry * 100; tp2_pct = abs(opp["tp2"] - entry) / entry * 100; tp3_pct = abs(opp["tp3"] - entry) / entry * 100
    liq, regime, session = opp["liq"], opp["regime"], opp.get("session", get_market_session()); vwap_lb, oi_lb, pos_hint = opp.get("vwap_lb", ""), opp.get("oi_lb", ""), opp.get("pos_hint", "─")
...
def format_alert(coin: str, side: str, alert_type: str, price: float, entry: float, sl: float, tp1: float, tp2: float, tp3: float, new_sl: float = None, score: int = 0) -> str:
    ...
    if alert_type == "ENTRY":
        sl_pct = abs(entry - sl) / entry * 100; tp1_pct = abs(tp1 - entry) / entry * 100; tp2_pct = abs(tp2 - entry) / entry * 100; tp3_pct = abs(tp3 - entry) / entry * 100; sl_sign = "-" if side == "LONG" else "+"
        ...
        pnl = ((price - entry) / entry * 100) if side == "LONG" else ((entry - price) / entry * 100); tp2_pct = abs(tp2 - entry) / entry * 100; tp3_pct = abs(tp3 - entry) / entry * 100; new_sl_str = f"`{new_sl:.4f}`" if new_sl else f"`{entry:.4f}`"
        ...
        pnl = ((price - entry) / entry * 100) if side == "LONG" else ((entry - price) / entry * 100); tp3_pct = abs(tp3 - entry) / entry * 100
        ...
        pnl = ((price - entry) / entry * 100) if side == "LONG" else ((entry - price) / entry * 100); is_be = new_sl is not None and abs(new_sl - entry) < entry * 0.0001; label = "保本止損" if is_be else "止損觸發"; header_em = "🛡" if is_be else "🛑"; sl_display = f"`{new_sl:.4f}`" if new_sl else f"`{sl:.4f}`"; r_tag = "`0.0R`" if is_be else "`-1.0R`"
...
def send_midnight_report(stats_file: str):
    today_str = tw_now().strftime('%Y-%m-%d'); date_str = (tw_now() - timedelta(days=1)).strftime('%Y-%m-%d')
    try:
        df_s = pd.read_csv(stats_file)
        if df_s.empty:
            send_tg(f"📊 *Alpha Oracle 每日戰績*\n──────────────────\n📅 日期：{date_str}\n\n📭 昨日無成交紀錄"); return
        tp_c = len(df_s[df_s['result']=='TP']); sl_c = len(df_s[df_s['result']=='SL']); total = tp_c + sl_c
        wr = (tp_c / total * 100) if total > 0 else 0; ev = (wr/100 * 1.5 + (1-wr/100) * (-1.0)) if total > 0 else 0
        send_tg(f"📊 *Alpha Oracle 每日戰績*\n──────────────────\n📅 日期：{date_str}\n\n✅ 盈利（含保本）：{tp_c} 單\n❌ 止損：{sl_c} 單\n📊 總計：{total} 單\n\n🔥 勝率：*{wr:.1f}%*\n💹 期望值：*{ev:+.2f}R / 單*\n──────────────────\n📌 保本亦計為獲勝\n💡 期望值 > 0 = 長期獲利策略")
    except Exception as e: logging.error(f"send_midnight_report 錯誤: {e}")
...
class SignalTracker:
    ...
    def _save(self):
        with open(self.filepath, "w", encoding="utf-8") as f: json.dump(self.signals, f, ensure_ascii=False, indent=2)
    def add(self, opp: dict, active: bool = False) -> str:
        key = f"{opp['instId']}_{opp['side']}_{opp['tf']}_{int(time.time())}"; now = time.time()
        with self._lock:
            self.signals[key] = {"instId": opp["instId"], "side": opp["side"], "tf": opp["tf"], "entry": opp["entry"], "sl": opp["sl"], "sl_orig": opp["sl"], "tp1": opp["tp1"], "tp2": opp["tp2"], "tp3": opp["tp3"], "score": opp["score"], "grade": opp["grade"], "status": "ACTIVE" if active else "PENDING", "hit_tp1": False, "hit_tp2": False, "touched_tp1": False, "touched_tp2": False, "alerted_touch_tp1": False, "alerted_touch_tp2": False, "created": now, "activated_at": now if active else None, "hit_tp1_at": None, "hit_tp2_at": None}
            self._save()
    ...
    def _close(self, sig: dict, close_price: float, close_type: str, stats_file: str = DAILY_STATS_FILE):
        if self.win_tracker:
        ...
        try:
            ...
            pd.DataFrame([{"instId": sig["instId"], "result": close_type, "date": today_str}]).to_csv(stats_file, mode='a', header=not os.path.exists(stats_file), index=False)
        except Exception as e: logging.error(f"記錄每日統計失敗: {e}")
    def _is_close_confirmed(self, sig: dict, level: float) -> bool:
        ...
        if df is None or len(df) < 1: logging.warning(f"  [{sig['instId']}] 收盤確認抓不到 K 線，暫不觸發 SL 移動"); return False
        last_close = float(df["c"].iloc[-1])
        if sig["side"] == "LONG": return last_close >= level
        else: return last_close <= level
    def check_one(self, key: str, sig: dict) -> bool:
        try:
            price = fetch_ticker_price(sig["instId"])
            if price <= 0: logging.warning(f"  [{key}] 無法取得即時價格，跳過檢查"); return False
            coin, side, status = sig["instId"].split("-")[0], sig["side"], sig["status"]; entry, sl = sig["entry"], sig["sl"]; tp1, tp2, tp3 = sig["tp1"], sig["tp2"], sig["tp3"]
            kline_high, kline_low, kline_last_close = price, price, price
            try:
                df_last = fetch_okx_last_closed(sig["instId"], tf=sig["tf"], limit=2)
                if df_last is not None and len(df_last) > 0: kline_high, kline_low, kline_last_close = float(df_last["h"].max()), float(df_last["l"].min()), float(df_last["c"].iloc[-1])
            except Exception as e: logging.warning(f"  [{key}] K 線 high/low 抓取失敗: {e}")
            ...
            if status == "PENDING":
                ...
                if age_h > SIGNAL_EXPIRE_HOURS: send_tg(f"⏰ *訊號過期* #{coin} {side}\n進場 `{entry:.4f}` 超過 {SIGNAL_EXPIRE_HOURS}h 未觸發"); logging.info(f"  [過期] {key}"); self.last_run_transitions += 1; return True
                if side == "LONG":
                    zone_lo, zone_hi = entry * (1 - ENTRY_TOLERANCE * 3), entry * (1 + ENTRY_TOLERANCE)
                    in_entry_zone = (zone_lo <= price <= zone_hi) or (zone_lo <= kline_low <= zone_hi) or (kline_low <= zone_hi and kline_high >= zone_lo)
                else:
                    zone_lo, zone_hi = entry * (1 - ENTRY_TOLERANCE), entry * (1 + ENTRY_TOLERANCE * 3)
                    in_entry_zone = (zone_lo <= price <= zone_hi) or (zone_lo <= kline_high <= zone_hi) or (kline_low <= zone_hi and kline_high >= zone_lo)
                if in_entry_zone:
                    self.update(key, status="ACTIVE", activated_at=time.time()); msg = format_alert(coin, side, "ENTRY", price, entry, sl, tp1, tp2, tp3, score=sig["score"])
                    if send_tg(msg): logging.info(f"  [進場] {key} @ {price:.4f} - 通知已發送")
                    else: logging.error(f"  [進場] {key} - 通知發送失敗")
            ...
            if sl_hit:
                is_be = (status in ("BE", "TRAIL") and abs(sl - entry) < entry * 0.0001); close_type = "BE" if is_be else "SL"; hit_price = sl
                msg = format_alert(coin, side, "SL", hit_price, entry, sig["sl_orig"], tp1, tp2, tp3, new_sl=(entry if is_be else sl))
                if send_tg(msg): logging.info(f"  [{close_type}] {key} @ {hit_price:.4f} (BE={is_be}) - 通知已發送")
                else: logging.error(f"  [{close_type}] {key} - 通知發送失敗")
                self._close(sig, hit_price, close_type); self.last_run_transitions += 1; return True
            ...
            if tp3_hit:
                ...
                if send_tg(msg): logging.info(f"  [TP3] {key} @ {tp3:.4f} ✅ 完美收割 - 通知已發送")
                else: logging.error(f"  [TP3] {key} - 通知發送失敗")
            ...
            if tp2_touched_now and not sig.get("hit_tp2"):
                if not sig.get("touched_tp2"):
                    self.update(key, touched_tp2=True); logging.info(f"  [TP2 觸及] {key}  價={price:.4f} H={kline_high:.4f} L={kline_low:.4f}  等收盤確認")
                    if not sig.get("alerted_touch_tp2"):
                        touch_msg = f"⚡ *TP2 觸及（待收盤確認）*  #{coin} {side}\n目標 `{tp2:.4f}` / 最高 `{kline_high:.4f}` / 最低 `{kline_low:.4f}`\n收盤站穩才會移損至 TP1"
                        if send_tg(touch_msg): self.update(key, alerted_touch_tp2=True)
                if not self._is_close_confirmed(sig, tp2): logging.info(f"  [TP2 待確認] {key}  本根 K 尚未收盤於 TP2 之上/下（close={kline_last_close:.4f}）"); return False
                now = time.time()
                if not sig.get("hit_tp1"): self.update(key, hit_tp1=True, touched_tp1=True, alerted_touch_tp1=True, hit_tp1_at=now); self._close(sig, tp1, "TP1")
                self.update(key, hit_tp2=True, sl=tp1, status="TRAIL", hit_tp2_at=now); msg = format_alert(coin, side, "TP2", tp2, entry, sig["sl_orig"], tp1, tp2, tp3, new_sl=tp1)
                if send_tg(msg): logging.info(f"  [TP2] {key} @ {tp2:.4f} → SL 移至 TP1={tp1:.4f} - 通知已發送")
                else: logging.error(f"  [TP2] {key} - 通知發送失敗")
            ...
            if tp1_touched_now and not sig.get("hit_tp1"):
                if not sig.get("touched_tp1"):
                    self.update(key, touched_tp1=True); logging.info(f"  [TP1 觸及] {key}  價={price:.4f} H={kline_high:.4f} L={kline_low:.4f}  等收盤確認")
                    if not sig.get("alerted_touch_tp1"):
                        touch_msg = f"⚡ *TP1 觸及（待收盤確認）*  #{coin} {side}\n目標 `{tp1:.4f}` / 最高 `{kline_high:.4f}` / 最低 `{kline_low:.4f}`\n收盤站穩才會移損至保本"
                        if send_tg(touch_msg): self.update(key, alerted_touch_tp1=True)
                if not self._is_close_confirmed(sig, tp1): logging.info(f"  [TP1 待確認] {key}  本根 K 尚未收盤於 TP1 之上/下（close={kline_last_close:.4f}）"); return False
                self.update(key, hit_tp1=True, sl=entry, status="BE", hit_tp1_at=time.time()); msg = format_alert(coin, side, "TP1", tp1, entry, sig["sl_orig"], tp1, tp2, tp3, new_sl=entry)
                if send_tg(msg): logging.info(f"  [TP1] {key} @ {tp1:.4f} → SL 移至保本={entry:.4f} - 通知已發送")
                else: logging.error(f"  [TP1] {key} - 通知發送失敗")
                self._close(sig, tp1, "TP1"); self.last_run_transitions += 1; return False
            return False
        except Exception as e: logging.error(f"check_one [{key}] 錯誤: {e}\n{traceback.format_exc()}"); return False
    def check_all(self):
    ...
    def status_summary(self) -> str:
        ...
        for idx, (key, s) in enumerate(items, 1):
            coin = s["instId"].split("-")[0]; arrow = "🟢" if s["side"] == "LONG" else "🔴"; side = s["side"]; em, st_desc = st_map.get(s["status"], ("❓", s["status"]))
            live = fetch_ticker_price(s["instId"]); entry = s["entry"]
            if s["status"] == "PENDING":
                if live > 0:
                    ...
                    zone_tag = "  ⚡ *已在進場區*" if in_zone else ""; price_line = f" 📍 當前 `{live:.4f}` (距離 `{dist_pct:+.2f}%`){zone_tag}"
                else: price_line = " 📍 當前 `—`"
            else:
                if live > 0: pnl = ((live - entry) / entry * 100) if side == "LONG" else ((entry - live) / entry * 100); sign = "+" if pnl >= 0 else ""; price_line = f" 💹 當前 `{live:.4f}` `{sign}{pnl:.2f}%`"
                else: price_line = " 💹 當前 `—`"
            if s["status"] == "BE": sl_label = f" 🛡 止損 `{s['sl']:.4f}` *(保本)*"
            elif s["status"] == "TRAIL": sl_label = f" 🛡 止損 `{s['sl']:.4f}` *(鎖利於 TP1)*"
            else: sl_label = f" 🛑 止損 `{s['sl']:.4f}`"
            ...
            lines.append(f"{em} *#{coin} · {arrow} {side} · {s['tf']}* [{s['score']}分]"); lines.append(f" {st_desc}"); lines.append(price_line); lines.append(f" 📌 進場 `{entry:.4f}`"); lines.append(sl_label); lines.append(f" 🥇 TP1 `{s['tp1']:.4f}`"); lines.append(f" 🥈 TP2 `{s['tp2']:.4f}`"); lines.append(f" 🏆 TP3 `{s['tp3']:.4f}`"); lines.append(f" 進度 {progress}")
            if idx < len(items): lines.extend(["", "─────────────────────────", ""])
        lines.extend(["", f"━━━━━━━━━━━━━━━━━━━━━━━━", f"🤖 Alpha Oracle v9.1.8 動態追蹤中"]); return "\n".join(lines)
...
def _check_entry_zone(opp: dict) -> tuple:
    """檢查價格是否已在進場區內（支援 tick + wick 雙重判斷）"""
    ...
    # 進場區間定義
    if side == "LONG":
        zone_lo = entry * (1 - tol * 3)
        zone_hi = entry * (1 + tol)
    else:
        zone_lo = entry * (1 - tol)
        zone_hi = entry * (1 + tol * 3)
    
    in_zone = zone_lo <= live <= zone_hi
    dist_pct = (live - entry) / entry * 100
...
def run_scan(tracker: SignalTracker) -> int:
    ...
    logging.info(f"═══ 掃描開始 閾值={SETUP_SCORE_THRESHOLD} 時框={SCAN_TIMEFRAMES} ═══"); all_opps: list = []; workers = min(5, len(ALL_COINS))
    ...
    for opp in all_opps:
        ...
        if not check_signal_cooldown(opp["instId"], opp["side"]): logging.info(f"  [{opp['instId']}/{opp['side']}] 冷卻期中（{SIGNAL_COOLDOWN_HOURS}h），跳過"); continue
        if send_tg(format_signal(opp)):
            sent += 1; set_signal_cooldown(opp["instId"], opp["side"]); logging.info(f"  #{sent} {opp['instId']} [{opp['tf']}]{opp['side']} {opp['score']}分")
...
def run_monitor_once(tracker: SignalTracker, push_status: bool = None) -> int:
    ...
    remaining = tracker.list_active(); transitions = getattr(tracker, "last_run_transitions", 0)
    if push_status is None: should_push = remaining and (transitions > 0 or _is_heartbeat_window())
...
# ─────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Alpha Oracle v9.1.8")
    parser.add_argument("--mode", default="all", choices=["scan","monitor","monitor_once","loop","all","daily_report","monthly_report"], help="scan=掃描 | monitor=持續監控 | monitor_once=單次監控 | loop=定時掃描+監控 | all=掃描+監控 | daily_report=今日戰報 | monthly_report=月度戰報")
    ...
    logging.info("=" * 60); logging.info("🤖 Alpha Oracle v9.1.8 啟動"); logging.info(f"📋 模式: {args.mode}"); logging.info(f"⏱ 監控間隔: {args.interval}秒"); logging.info(f"🎯 TP 收盤確認: {CONFIRM_TP_ON_CLOSE}"); logging.info(f"🐛 除錯模式: {os.getenv('DEBUG_MODE', 'false')}"); logging.info("=" * 60)
    now_tw = tw_now(); is_midnight = (now_tw.hour == 0 and 0 <= now_tw.minute < 15)
    if is_midnight:
        try: send_midnight_report(DAILY_STATS_FILE); pd.DataFrame(columns=["instId", "result", "date"]).to_csv(DAILY_STATS_FILE, index=False); logging.info("✅ 午夜戰報已發送，統計已重置")
        except Exception as e: logging.error(f"午夜戰報發送失敗: {e}")
